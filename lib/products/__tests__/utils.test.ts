...
// ─── parseSort ────────────────────────────────────────────────────────────────
describe("parseSort", () => {
  it("devuelve orden por precio ascendente", () => {
    expect(parseSort("price_asc")).toEqual([{ priceCents: "asc" }]);
  });

  it("devuelve orden por precio descendente", () => {
    expect(parseSort("price_desc")).toEqual([{ priceCents: "desc" }]);
  });

  it("devuelve orden por nombre ascendente", () => {
    expect(parseSort("name_asc")).toEqual([{ name: "asc" }]);
  });

  it("devuelve orden por fecha descendente", () => {
    expect(parseSort("date_desc")).toEqual([{ createdAt: "desc" }]);
  });

  it("devuelve orden por defecto para valor desconocido", () => {
    expect(parseSort("unknown")).toEqual([
      { sortOrder: "asc" },
      { createdAt: "desc" },
    ]);
  });
...
describe("parseSearchParamFilters", () => {
  ...
  it("parsea sort correctamente", () => {
    expect(parseSearchParamFilters({ sort: "price_asc" }).sort).toEqual([
      { priceCents: "asc" },
    ]);
  });
});
