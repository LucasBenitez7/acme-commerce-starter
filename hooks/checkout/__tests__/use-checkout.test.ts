...
vi.mock("@/app/(site)/(shop)/checkout/actions", () => ({
  createOrderAction: (...args: unknown[]) => mockCreateOrderAction(...args),
}));
...
describe("useCheckout", () => {
  ...
  it("onConfirmAddress limpia localStorage si createOrderAction devuelve error", async () => {
    ...
    mockCreateOrderAction.mockResolvedValue({ error: "Error al procesar" });
    const { result } = renderHook(() => useCheckout([]));
    await act(async () => {
      await result.current.onConfirmAddress();
    });
