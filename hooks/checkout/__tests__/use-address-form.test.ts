...
vi.mock("@/app/(site)/(account)/account/addresses/actions", () => ({
  upsertAddressAction: (...args: unknown[]) => mockUpsertAddressAction(...args),
}));
...
describe("useShippingAddressForm", () => {
  ...
  describe("flujo usuario registrado (isGuest: false)", () => {
    it("llama a upsertAddressAction con los datos del formulario", async () => {
      mockUpsertAddressAction.mockResolvedValue({
    ...
    it("llama a onSuccess con la dirección devuelta por la action", async () => {
      mockUpsertAddressAction.mockResolvedValue({
        address: mockSavedAddress,
      });
      const onSuccess = vi.fn();
    ...
    it("no llama a onSuccess si upsertAddressAction devuelve error", async () => {
      mockUpsertAddressAction.mockResolvedValue({ error: "Error al guardar" });
      const onSuccess = vi.fn();
      const { result } = renderHook(() =>
        useShippingAddressForm({ ...defaultProps, onSuccess }),
      );
    ...
    it("pasa el id de initialData si se está editando una dirección existente", async () => {
      mockUpsertAddressAction.mockResolvedValue({ address: mockSavedAddress });
      const { result } = renderHook(() =>
        useShippingAddressForm({
          ...defaultProps,
          initialData: mockSavedAddress,
