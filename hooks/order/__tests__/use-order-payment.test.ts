...
const { mockGetPaymentIntentAction } = vi.hoisted(() => ({
  mockGetPaymentIntentAction: vi.fn(),
}));

vi.mock("@/app/(site)/(account)/account/orders/actions", () => ({
  getPaymentIntentAction: mockGetPaymentIntentAction,
}));
...
describe("useOrderPayment", () => {
  ...
  it("abre el modal inmediatamente al llamar startPaymentFlow", async () => {
    mockGetPaymentIntentAction.mockResolvedValue({
  ...
  it("llama a getPaymentIntentAction con el orderId correcto", async () => {
    ...
    expect(mockGetPaymentIntentAction).toHaveBeenCalledWith("order-1");
  });
  ...
  it("no llama a getPaymentIntentAction si ya hay clientSecret", async () => {
    ...
    expect(mockGetPaymentIntentAction).not.toHaveBeenCalled();
    expect(result.current.isOpen).toBe(true);
  });
